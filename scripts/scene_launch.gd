...
func _ready():
	...
	get_tree().get_root().get_child(0).add_child(player_one_instance)
	get_tree().get_root().get_child(0).add_child(player_two_instance)
	if first_scene:
		winner_heavy = false
		winner_big = false
		...
		if winner_heavy:
			randomize()
			player_one_instance.heavy = randf() > (0.5 - heavy_modifier)
			player_two_instance.heavy = randf() > (0.5 - heavy_modifier)
		else:
			randomize()
			player_one_instance.heavy = randf() > (0.5 + heavy_modifier)
			player_two_instance.heavy = randf() > (0.5 + heavy_modifier)
		if winner_big:
			randomize()
			player_one_instance.big = randf() > (0.5 - big_modifier)
			player_two_instance.big = randf() > (0.5 - big_modifier)
		else:
			randomize()
			player_one_instance.big = randf() > (0.5 + big_modifier)
			player_two_instance.big = randf() > (0.5 + big_modifier)
		if winner_sworded:
			randomize()
			player_one_instance.has_sword = randf() > (0.5 - sworded_modifier)
			player_two_instance.has_sword = randf() > (0.5 - sworded_modifier)
		else:
			randomize()
			player_one_instance.has_sword = randf() > (0.5 + sworded_modifier)
			player_two_instance.has_sword = randf() > (0.5 + sworded_modifier)
...
func on_player_death(var player_number):
	var winner = get_tree().get_root().get_child(0).get_node("player_" + String(player_number))
